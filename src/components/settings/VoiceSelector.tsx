...
/// Silero voice ids are `<language>_<name>`. The multilingual packages carry
/// speakers for every CIS language in one model, so the prefix is the only way
/// to tell them apart in the list.
const SILERO_LANGUAGES: Record<string, string> = {
  ru: "Russian",
  bak: "Bashkir",
  tat: "Tatar",
  kaz: "Kazakh",
...
/// OpenAI voice ids are bare names with no language or gender encoded, so they
/// are listed as-is; the model speaks whatever language the text is in.
...
function getVoiceLabel(voiceId: string): string {
  ...
  if (OPENAI_VOICES.has(voiceId)) {
    return capitalize(voiceId);
  }

  const parts = voiceId.split("_");
  if (parts.length >= 2) {
    const name = capitalize(parts.slice(1).join("_"));
    const language = SILERO_LANGUAGES[parts[0]];
    // Silero does not publish a gender per speaker, so the label stays at
    // name + language rather than guessing one.
...
export const VoiceSelector: React.FC<VoiceSelectorProps> = ({
  ...
  const selectedVoice = getSetting("selected_voice") ?? null;

  const loadVoices = useCallback(async () => {
    try {
      const result = await commands.getAvailableVoices();
      if (result.status === "ok") {
  ...
  const dropdownOptions: DropdownOption[] = useMemo(() => {
    return [
      {
        value: "__auto__",
        label: t("settings.modelSettings.voice.auto"),
      },
      ...voices.map((voice) => ({
        value: voice,
  ...
  const handleSelectVoice = async (value: string) => {
    ...
    await updateSetting("selected_voice", voiceValue);
  };

  const handleReset = async () => {
    await resetSetting("selected_voice");
  };

  const selectedValue = selectedVoice ?? "__auto__";
  const disabled = isUpdating("selected_voice");

  return (
    <SettingContainer
      title={t("settings.modelSettings.voice.title")}
      description={t("settings.modelSettings.voice.description")}
    ...
    >
      <div className="flex items-center space-x-1">
        <Dropdown
          ...
          placeholder={t("settings.modelSettings.voice.auto")}
        />
