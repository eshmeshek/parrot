...
const SUPPORTED_ENGINE_TYPES: ModelInfo["engine_type"][] = ["Kokoro", "Silero", "OpenAi"];
...
export const ModelsSettings: React.FC = () => {
  ...
  const getModelStatus = (modelId: string): ModelCardStatus => {
    ...
    const model = filteredEngineModels.find((m: ModelInfo) => m.id === modelId);
  ...
  const filteredModels = useMemo(() => {
    return filteredEngineModels.filter((model: ModelInfo) => {
      if (languageFilter !== "all") {
        if (!modelSupportsLanguage(model, languageFilter)) return false;
      }
      return true;
    });
  }, [filteredEngineModels, languageFilter]);
